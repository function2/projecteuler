...
// Note: We could solve this with bignums but that would defeat the purpose. They want the 10 most significant digits.
...
static const int NUM_NUMS = 100;
static const int NUM_DIGITS = 50;

const byte TABLE[NUM_NUMS][NUM_DIGITS] = {
...
int main()
{
	// The result couldn't possibly go into larger digits.
	const int N = NUM_DIGITS+3;

	// Add up all digit * 10^k into sums[] entries.
	int sums[N] = {0};
	for(int j = NUM_DIGITS-1;j >= 0;--j)
		for(int k = 0;k < NUM_NUMS;++k)
			sums[NUM_DIGITS-1 - j] += TABLE[k][j];

	// Do the addition with carryover.
	for(int k = 0;k < N-1; ++k){
		sums[k+1] += sums[k]/10;
		sums[k] %= 10;
	}
